...
- **Воспроизводимость:** `Makefile`, `Dockerfile`, `.env.example`, `requirements.lock`, 47 офлайн-тестов `pytest`, включая сквозной прогон агента, проверку fallback при недоступной LLM, проверку фактов в тексте, рельеф и прототип калибровки из исследования R2. Замеры чистой установки — в [аудите R4](docs/research/R4_audit.md).
...
| `tests/` | 47 офлайн-тестов: часовой пояс, SCADA, архивный прогноз «как в прошлом», признаки и рельеф, модель, сквозной прогон агента без LLM и со сбоем LLM (fallback на правила, те же числа), проверка фактов в нарративе, прототип калибровки (R2) |
...
| Цель | Команда |
|---|---|
| `make test` | `.venv/bin/python -m pytest -q` (офлайн; 47 тестов, время зависит от машины) |
...
# 2. тесты (офлайн; 47 тестов, 12–67 с в аудите)
pytest -q
