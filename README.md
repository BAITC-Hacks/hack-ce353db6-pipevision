...
**Панель оператора:** [Railway](https://windagent-nurly-production.up.railway.app) — существующая публичная версия. Описание ниже относится к обновлению в этом PR; до его слияния и деплоя публичный сайт может показывать прежний интерфейс.

В рабочем пространстве «Оператор» основной экран содержит три KPI, график мощности и компактную схему «Погода → Данные → Прогноз → Проверка». Риски и пояснения привязаны к выбранному выпуску и горизонту 24/48 ч. Подробные объяснения, диалог с агентом, таблицы, качество модели и технический журнал открываются по запросу. Реальные стадии расчёта показываются без имитации; короткая анимация подтверждает явный расчёт или включение AI-пояснений, затем прекращается. Постоянного видео и декоративной анимации нет; reduced-motion учитывается. Карта, атлас и 3D-сцена остаются в отдельном пространстве «Исследование площадки».
