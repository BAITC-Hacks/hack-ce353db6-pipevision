...
| Цель | Команда |
|---|---|
| `make test` | `.venv/bin/python -m pytest -q` (офлайн; 69 тестов, время зависит от машины) |
...
# 2. тесты (офлайн; 69 тестов, около 15 с на macOS; 47 тестов ядра шли 12–67 с в аудите R4)
pytest -q
...
**Панель оператора:** https://windagent-nurly-production.up.railway.app — развёрнута на Railway из `deploy/Dockerfile.ui` (Streamlit; см. [`ui/README.md`](ui/README.md)). Сверху — карточка агента: ход цикла по шагам с таймингами, решение, сверка чисел, сообщение агента и диалог с ним по текущему выпуску или площадке (LLM с проверкой каждого числа; без ключа — ответы по правилам). Ниже два рабочих пространства. «Оператор»: статус выпуска, риски и предупреждения, KPI, веер P10–P90, «Почему такой прогноз», изменения к выпуску D−1, экспорт, 3D-сцена «Нурлы», таблицы, тестовый период, факт и точность, журнал агента; все блоки считаются по выбранному горизонту 24/48 ч. «Исследование площадки»: 3D-сцена и карта Казахстана с атласом ветра; клик по ячейке переносит координаты в форму, кнопка «Исследовать площадку» запускает оценку ресурса (ERA5, Вейбулл, КИУМ, годовая выработка, сравнение с «Нурлы»), строит рельеф и 3D-сцену точки, прогноз на 48 ч переносом модели и отчёт для руководства. В образе лежат данные, кэш архивных прогнозов, атлас и модель, поэтому ретроспектива по ВЭС «Нурлы» работает без внешних запросов; live-режим и исследование других площадок обращаются к Open-Meteo. LLM-режим на сервере включается переменной `OPENAI_API_KEY` в настройках сервиса. Локально: `make setup-ui && make ui` (http://localhost:8501) или `docker build -f deploy/Dockerfile.ui -t wind-agent-ui . && docker run --rm -p 8501:8501 wind-agent-ui`.
